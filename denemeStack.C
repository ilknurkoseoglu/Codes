...
void denemeStack(TString samp)
{
   ...
   ////
    ...
    THStack *hs1=new THStack("hs1","Delphes Simulation	       			    													 14TeV,   300 fb^{-1}  ");
    	
    
        
    TH1F* Element_DiBoson;
    TFile *bkg1 = TFile::Open("DeepSingle+DelphMET_NoPU_DiBoson_his.root");
...
*/
    TH1F* ML4;
    TFile *bkg6 = TFile::Open("DeepSingle+DelphMET_NoPU_ML4_his.root");
    ML4= (TH1F*) bkg6->Get(samp);
    ML4->SetLineStyle(2);
    ML4->SetLineWidth(3);
    ML4->SetLineColor(kBlack);

    
    Double_t norm1 = Element_DiBoson->GetEntries();
    Element_DiBoson->Scale(1/norm1);
 //   Signal2->SetStats(0); 
    Double_t norm2 = Element_BosonJets->GetEntries();
    Element_BosonJets->Scale(1/norm2);
   // Signal3->SetStats(0);
    Double_t norm3 = Element_TopJets->GetEntries();
    Element_TopJets->Scale(1/norm3);

    Double_t norm5 = Element_TTbar->GetEntries();
    Element_TTbar->Scale(1/norm5);

    Double_t norm4 = ML4->GetEntries();
    ML4->Scale(1/norm4);
...
//		pl->AddEntry(ML5, "ML5","lpf" );
    ///*ple=*/pl->AddEntry("Sceneario NoPU");
    pl->SetFillColor(0);
    pl->SetLineColor(0.5);
    pl->Draw("same");
    
  //  c1->SaveAs("StackPlots/stack_50PU"+samp+"300lumi.pdf");
    c1->SaveAs("StackPlots_NoPU_ML4/stack_NoPU_"+samp+"_ML4_300lumi.root");
    c1->SaveAs("StackPlots_NoPU_ML4/stack_NoPU_"+samp+"_ML4_300lumi.png");
}
